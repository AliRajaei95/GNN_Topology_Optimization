...
framework with multiple switching. The case study evaluates the runtime performance of machine-learning-
...
for seed in range(0,Num_seeds):
    ...
    case_study_path = Folder+f'run_results/cs5multi{max_sw}_{system}bus_ClfReg_{modeltype}_{topk}top_{traintype}_to_same_{dataname}_gamma{gamma}_hops{hops}_layer{layer}_hid{hid}_seed{seed}.pkl'
    ...
    cs_res = CaseStudy5_Time_Clf_Reg(networkdata,Clfres['test_res'],Regres['test_res'],
                                     modeltype=modeltype,TopMLActions=topk, Max_Sw_bus=max_sw )
