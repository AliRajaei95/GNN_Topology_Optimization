...
control (AC formulation).

This script reproduces the computational-efficiency experiments presented
in Section IV-E of the paper for the AC optimal substation switching (AC-OSS)
...
exec(open(Folder+'AC_Optimal_Splitting_Functions_Pyomo.py').read())
...
traintype='N0'
system='118'
dataname='dataset3_gen' #dataset1_1lim, dataset3_gen
...
for seed in range(0,Num_seeds):
    ...
    case_study_path = Folder+f'run_results/cs5_AC_{system}bus_ClfReg_{modeltype}_{topk}top_{traintype}_to_same_{dataname}_gamma{gamma}_hops{hops}_layer{layer}_hid{hid}_seed{seed}.pkl'
    ...
    cs_res = CaseStudy5_AC_Time_Clf_Reg(networkdata,Clfres['test_res'],Regres['test_res'],
                                     modeltype=modeltype,TopMLActions=topk,PQfactor = 0.3  )
